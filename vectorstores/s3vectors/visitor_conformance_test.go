...
func TestVisitor_Conformance(t *testing.T) {
	storetest.VisitorConformance(t, func(src string) error {
		...
		storetest.Options{
			// The documented operator set has no pattern match. Filtered
			// deletion enumerates with ListVectors and decides membership
			// with filter.Match, so LIKE works there — Search compiles a
			// server-side filter and cannot.
