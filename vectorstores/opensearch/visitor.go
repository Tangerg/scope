...
func (v *visitor) visitComparisonExpr(expr *filter.BinaryExpr) error {
	...
	case filter.OpGreaterEqual:
		...
		v.sql.WriteString(term)
	default:
		return fmt.Errorf("opensearch: unexpected comparison operator '%s'", expr.Operator().String())
	}
	return nil
}
...
func formatLiteral(lit *filter.Literal) (string, error) {
	...
	case lit.IsString():
		...
		if err != nil {
			return "", fmt.Errorf("opensearch: %w (at %s)", err, lit.Start().String())
		}
	...
	default:
		return "", fmt.Errorf("opensearch: unsupported literal kind '%s' at %s",
