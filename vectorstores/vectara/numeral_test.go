...
func TestVisitorRendersExactNumerals(t *testing.T) {
	tests := []struct {
		...
		{name: "int64 boundary", expr: filter.EQ("n", float64(1<<63)), want: `doc.n = 9223372036854776000`},
		{name: "past int64", expr: filter.EQ("n", uint64(1<<64-1)), want: `doc.n = 18446744073709551615`},
		{name: "integral float", expr: filter.EQ("n", 1000000.0), want: `doc.n = 1000000`},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			visitor := newVisitor("doc")
