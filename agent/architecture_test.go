...
func TestReceiverMethodsStayWithTheirType(t *testing.T) {
	...
	err := filepath.WalkDir(".", func(path string, entry fs.DirEntry, walkErr error) error {
		...
		for _, declaration := range file.Decls {
			...
			case *ast.FuncDecl:
				...
				key := receiverKey{filepath.Dir(path), file.Name.Name, receiverTypeName(declaration.Recv)}
				methods = append(methods, methodLocation{key, declaration.Name.Name, files.Position(declaration.Pos())})
			}
		}
		return nil
	})
	...
	for _, method := range methods {
		...
		if method.position.Filename != owner {
