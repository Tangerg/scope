...
func TestPauseWaitingSurvivesRestoreAndRequiresResume(t *testing.T) {
	for _, answerBeforeResume := range []bool{false, true} {
		t.Run(fmt.Sprintf("answer_before_resume_%t", answerBeforeResume), func(t *testing.T) {
...
type waitingAcknowledgmentCommitter struct {
	*MemoryTreeCommitter
	waitingCommits int
	lost           TreeCheckpoint
	failure        error
...
func (w *waitingAcknowledgmentCommitter) CommitCheckpoint(ctx context.Context, checkpoint TreeCheckpoint) error {
	if err := w.MemoryTreeCommitter.CommitCheckpoint(ctx, checkpoint); err != nil {
		return err
	}
	if checkpoint.TreeSnapshot().ProcessSnapshots()[0].Status() == StatusWaiting {
		w.waitingCommits++
		if w.waitingCommits == 2 {
			w.lost = checkpoint
			return w.failure
		}
	}
...
func TestWaitingCycleRecoversLostAcknowledgmentFromAuthoritativeHead(t *testing.T) {
	failure := errors.New("waiting acknowledgment lost")
	committer := &waitingAcknowledgmentCommitter{MemoryTreeCommitter: NewMemoryTreeCommitter(), failure: failure}
	...
	if err := process.Join(t.Context()); !errors.Is(err, failure) {
		t.Fatalf("lost acknowledgment Join: %v", err)
	}
	...
	if err := committer.MemoryTreeCommitter.CommitCheckpoint(t.Context(), committer.lost); err != nil {
		t.Fatalf("lost acknowledgment retry: %v", err)
	}
	mustCloseEngine(t, engine)
	restoredEngine := controlValue(NewEngine(EngineConfig{TreeCommitter: committer.MemoryTreeCommitter}))
	restored := controlValue(restoredEngine.RestoreTree(t.Context(), deployment, head))
	waitForStatus(t, restored, StatusWaiting)
	if err := committer.MemoryTreeCommitter.CommitCheckpoint(t.Context(), committer.lost); !errors.Is(err, ErrTreeIncarnationConflict) {
		t.Fatalf("retired writer retry: %v", err)
	}
	if err := restored.Pause(t.Context(), "review"); err != nil {
		t.Fatal(err)
	}
	waitForStatus(t, restored, StatusPaused)
	if err := restored.Resume(t.Context()); err != nil {
		t.Fatal(err)
	}
	waitForStatus(t, restored, StatusWaiting)
	if err := restored.Kill(t.Context(), "cleanup"); err != nil {
		t.Fatal(err)
	}
	awaitResult(t, restored)
